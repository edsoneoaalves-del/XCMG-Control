...
function linhaRegistro(r,acoes){const data=(r.inicio||'').split('-').reverse().join('/');const funcao=r.funcao_colaborador||r.funcao||'';const badgeAuto=r.origem_programacao?(programacoesAtivasNaData($('dataPainel')?.value||hoje()).some(p=>chavePessoaPeriodo(p)===chavePessoaPeriodo(r))?'Automático':'Histórico'):'Histórico';return `<div class="record-row ${classeTipo(r.tipo)}" data-record-id="${r.id}"><div class="record-cell cell-type"><span class="tag">${escapar(r.tipo)}</span></div><div class="record-cell cell-person"><strong>${escapar(nomeCompletoRegistro(r))}</strong><small>${[r.matricula,funcao].filter(Boolean).map(escapar).join(' • ')}</small></div><div class="record-cell cell-local">${escapar(r.local||'—')}</div><div class="record-cell cell-date" data-short-date="${escapar(data?data.slice(0,6)+data.slice(-2):'—')}">${escapar(data||'—')}</div><div class="record-cell cell-detail" title="${escapar(detalheRegistro(r))}">${escapar(detalheRegistro(r)||'—')}</div><div class="record-cell cell-attachment">${r.foto_url?`<a class="record-attachment" href="${escapar(r.foto_url)}" target="_blank" rel="noopener">📎 Anexo</a>`:'—'}</div>${acoes?`<div class="record-cell cell-actions"><button class="secondary" data-edit="${r.id}">Editar</button><button class="danger" data-delete="${r.id}">Excluir</button></div><details class="record-mobile-menu"><summary aria-label="Ações do registro">⋮</summary><div><button class="secondary" data-edit="${r.id}">Editar</button><button class="danger" data-delete="${r.id}">Excluir</button></div></details>`:`<div class="record-cell cell-actions"><span class="history-only-badge">${badgeAuto}</span></div><span class="record-mobile-auto">${badgeAuto}</span>`}</div>`}
